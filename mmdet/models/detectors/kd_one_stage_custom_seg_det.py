...
class CustomKnowledgeDistillationMultiTaskNetwork(SingleStageDetector):
    ...
    def forward_train(self,
        ...
        """
        x, x_semantic = self.extract_feat(img)
        # x_semantic = x[:4]
        # x = x[1:]
        # x_semantic = x
        ...
        if self.with_semantic_head: 
            semantic_loss = self.semantic_head.forward_train(x_semantic, gt_semantic_seg)
        ...
        if self.debug:
            ...
            for i, img_meta in enumerate(img_metas):
                ...
                imshow_bboxes(img_vis, gt_bboxes[i].cpu().numpy(), show=False, out_file='debug_files/bboxes' + str(i) + '.png')
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        """
        feat, feat_semantic = self.extract_feat(img)
        # feat_semantic = feat[:4]
        # feat = feat[1:]
        ...
        seg_preds = self.semantic_head.simple_test(feat_semantic, img_metas, rescale)
    ...
    def with_teacher(self):
        return hasattr(self,
                       'teacher_model') and self.teacher_model is not None
    
    @property
    def with_neck2(self):
        """bool: whether the detector has a neck"""
        return hasattr(self, 'neck2') and self.neck2 is not None